...
#include "rsyn/qt/graphics/util/GraphicsStipple.h"
...
#include "rsyn/qt/graphics/infra/GraphicsItem.h"
...
#include "rsyn/qt/graphics/view/layout/LayoutGraphicsScene.h"
#include "rsyn/qt/graphics/view/layout/LayoutGraphicsView.h"
...
MainWindow::saveCommandHistory() {
	...
	for (int i = start; i < length; i++, counter++) {
		...
		if (!cmd.empty()) {
			clsSettings.setValue("command", QString::fromStdString(cmd));
		} // end if
	} // end for
	clsSettings.endArray();
} // end method
...
MainWindow::createCommands() {
	...
	{ // Command: clearDrawings
		...
		dscp.setDescription("Draw a circle around a point of interest.");
		...
		session.registerCommand(dscp, [&](const ScriptParsing::Command &command) {
			if (scene) {
				...
				const QRectF mappedTypicalRect = view->mapToScene(typicalRect).boundingRect();
				...
				drawing.lineTo(rect.x() + rect.width(), rect.y() + rect.height());

				drawing.moveTo(rect.x(), rect.y() + rect.height());
				drawing.lineTo(rect.x() + rect.width(), rect.y());
...
MainWindow::populate() {
	...
	{ // Initialize view
		...
		// TODO: Make the graphics view get the keyboard focus. The following
		// is not working.
		view->activateWindow();
		view->setFocus();
	} // end block
...
void
MainWindow::onShowConnectivity(bool enable) {
...
void
MainWindow::onShowRoutingGuides(bool enable) {
...
MainWindow::onSearch() {
	...
	// Temporary workaround while still have the two graphics infrastructures.
	LayoutGraphicsScene *layoutScene = dynamic_cast<LayoutGraphicsScene *>(scene);
	if (!layoutScene)
		return;
	...
	if (instance) {
		std::cout << "Instance " << instance.getName() << " selected.\n";
		QRectF bounds;
		if (success = layoutScene->selectInstance(instance, bounds)) {
			view->ensureVisible(bounds);
		} else {
			std::cout << "BUG: Unable to find instance in the scene.\n";
		}
	} else {
    ...
    if (obj == txtCommand)  {
        ...
        if (event->type() == QEvent::KeyPress) {
            ...
            if (keyEvent->key() == Qt::Key_Up) {
				...
				txtCommand->setText(txtCommand->text() + QString::fromStdString(remaining));
				historyChanged = true;
			} // end else-if
